export type Movie = {
    id: number;
    title: string;
    poster: string;
    genres: string[];
    imdb: number;
    releaseDate: string;
    description: string;
    kaan?: boolean
}

export const movies: Movie[] = [
    {
        id: 1,
        title: "A Working Man",
        poster: "/images/posters/a-working-man.webp",
        genres: ["Aksiyon", "Suç", "Gerilim"],
        imdb: 6.1,
        releaseDate: "28.03.2025",
        description: "Chuck Dixon'ın 2014 yılında yayımlanan Levon's Trade adlı romanından uyarlanmıştır. Levon Cade, eski bir Kraliyet Deniz Komandosu'dur ve artık inşaat işçisi olarak sakin bir yaşam sürmektedir. Ancak patronunun genç kızı Jenny'nin insan kaçakçıları tarafından kaçırılmasıyla birlikte, Levon eski yeteneklerini kullanmak zorunda kalır. Jenny'yi bulmak için çıktığı bu yolculuk, onu büyük bir yolsuzluk ve hükümet ajanlarının insan kaçakçılığına karıştığı bir komploya götürür."
    },
    {
        id: 2,
        title: "Kayıp Dünya",
        poster: "/images/posters/kayıp-dunya.webp",
        genres: ["Fantastik", "Macera", "Aksiyon"],
        imdb: 7,
        releaseDate: "25.04.2025",
        description: "Bu fantastik macera, aynı adı taşıyan George R.R. Martin öykülerinden uyarlanmıştır. Filmin yönetmeni Paul W.S. Anderson’dır. Bir büyücü, Kayıp Diyarlar’a doğru yolculuk eder. Amacı, bir kişinin kurt adam dönüşümüne olanak tanıyan sihirli bir gücü bulmaktır. Bu mistik hikâye, iyilik ve kötülük, borç ve tatmin, aşk ve kayıp gibi temaları keşfeder. Büyü ve yaratıklarla dolu bu dünyada, büyücü Alys ve rehberi, gezgin Boyce, insanlar ve iblislerle mücadele ederken zorlu bir yolculuğa çıkarlar. Büyülü bir dünyada gerilim ve macera dolu bir yolculuğa katılın."
    },
    {
        id: 3,
        title: "Bir Minecraft Filmi",
        poster: "/images/posters/minecraft.webp",
        genres: ["Aile", "Komedi", "Macera"],
        imdb: 8,
        releaseDate: "04.04.2025",
        description: "Dört uyumsuz, gizemli bir portaldan aniden Üst Dünya'ya çekildiklerinde kendilerini sıradan sorunlarla boğuşurken bulurlar: hayal gücüyle beslenen tuhaf, kübik bir harikalar diyarı. Eve geri dönmek için, beklenmedik, uzman bir zanaatkar olan Steve ile büyülü bir göreve çıkarken bu dünyada ustalaşmaları gerekecektir."
    },
    {
        id: 4,
        title: "Moana 2",
        poster: "/images/posters/moana-2.webp",
        genres: ["Animasyon", "Macera", "Aile"],
        imdb: 7.2,
        releaseDate: "29.11.2024",
        description: "Moana ve Maui'yi üç yıl sonra alışılmadık denizcilerden oluşan bir ekiple birlikte geniş kapsamlı yeni bir yolculuk için yeniden bir araya getiriyor. Yol gösterici atalarından beklenmedik bir çağrı alan Moana, daha önce karşılaşmadığı bir macera için Okyanusya'nın uzak denizlerine, tehlikeli ve uzun süredir bilinmeyen sularına yolculuk etmek zorunda."
    },
    {
        id: 5,
        title: "Oppenheimer",
        poster: "/images/posters/oppenheimer.webp",
        genres: ["Dram", "Tarih"],
        imdb: 7.2,
        releaseDate: "21.07.2023",
        description: "Atom bombasının yaratıcısı J. Robert Oppenheimer’ın yaşamını konu alır."
    },
    {
        id: 6,
        title: "Dune: Çöl Gezegeni",
        poster: "/images/posters/col-gezegeni.webp",
        genres: ["Bilim-Kurgu", "Macera"],
        imdb: 7.2,
        releaseDate: "22.10.2021",
        description: "Çöl gezegeni Arrakis’te geçen epik bilim kurgu destanı."
    },
    {
        id: 7,
        title: "Moana 2",
        poster: "/images/posters/moana-2.webp",
        genres: ["Animasyon", "Macera", "Aile"],
        imdb: 7.2,
        releaseDate: "29.11.2024",
        description: "Moana ve Maui'yi üç yıl sonra alışılmadık denizcilerden oluşan bir ekiple birlikte geniş kapsamlı yeni bir yolculuk için yeniden bir araya getiriyor. Yol gösterici atalarından beklenmedik bir çağrı alan Moana, daha önce karşılaşmadığı bir macera için Okyanusya'nın uzak denizlerine, tehlikeli ve uzun süredir bilinmeyen sularına yolculuk etmek zorunda."
    },
    {
        id: 8,
        title: "A Working Man",
        poster: "/images/posters/a-working-man.webp",
        genres: ["Aksiyon", "Suç", "Gerilim"],
        imdb: 6.1,
        releaseDate: "28.03.2025",
        description: "Chuck Dixon'ın 2014 yılında yayımlanan Levon's Trade adlı romanından uyarlanmıştır. Levon Cade, eski bir Kraliyet Deniz Komandosu'dur ve artık inşaat işçisi olarak sakin bir yaşam sürmektedir. Ancak patronunun genç kızı Jenny'nin insan kaçakçıları tarafından kaçırılmasıyla birlikte, Levon eski yeteneklerini kullanmak zorunda kalır. Jenny'yi bulmak için çıktığı bu yolculuk, onu büyük bir yolsuzluk ve hükümet ajanlarının insan kaçakçılığına karıştığı bir komploya götürür."
    },
    {
        id: 9,
        title: "Oppenheimer",
        poster: "/images/posters/oppenheimer.webp",
        genres: ["Dram", "Tarih"],
        imdb: 7.2,
        releaseDate: "21.07.2023",
        description: "Atom bombasının yaratıcısı J. Robert Oppenheimer’ın yaşamını konu alır."
    },
    {
        id: 10,
        title: "Kayıp Dünya",
        poster: "/images/posters/kayıp-dunya.webp",
        genres: ["Fantastik", "Macera", "Aksiyon"],
        imdb: 7,
        releaseDate: "25.04.2025",
        description: "Bu fantastik macera, aynı adı taşıyan George R.R. Martin öykülerinden uyarlanmıştır. Filmin yönetmeni Paul W.S. Anderson’dır. Bir büyücü, Kayıp Diyarlar’a doğru yolculuk eder. Amacı, bir kişinin kurt adam dönüşümüne olanak tanıyan sihirli bir gücü bulmaktır. Bu mistik hikâye, iyilik ve kötülük, borç ve tatmin, aşk ve kayıp gibi temaları keşfeder. Büyü ve yaratıklarla dolu bu dünyada, büyücü Alys ve rehberi, gezgin Boyce, insanlar ve iblislerle mücadele ederken zorlu bir yolculuğa çıkarlar. Büyülü bir dünyada gerilim ve macera dolu bir yolculuğa katılın."
    }
]
